...
-spec websocket_init(ControllerData :: map()) -> {reply, {text, iodata()}, map()} | {ok, map()}.
...
websocket_handle({text, JSONBinary}, ControllerData) ->
    try
        ...
        case maps:get(~"type", Message, undefined) of
            ...
            _ ->
                Err = json_encode(#{type => ~"error", message => ~"Unknown message type"}),
                {reply, {text, Err}, ControllerData}
        end
    catch
        _:_:_ ->
...
websocket_info({pubsub_message, ~"arizona:reload", FileType}, ControllerData) ->
    Payload = json_encode(#{type => ~"reload", file_type => FileType}),
    {reply, {text, Payload}, ControllerData};
...
view_resolver(#{path := <<"/">>}) ->
    {view, esc_home_view, undefined, []};
view_resolver(#{path := <<"/modules/", _/binary>>}) ->
    {view, esc_module_view, undefined, []};
